...
try:
    ...
    def get_embedding(text: str) -> List[float]:
        text = text.replace("\n", " ")
        return (
            _client.embeddings.create(input=[text], model=_emb_model).data[0].embedding
        )
...
def get_invoice_payments(params: InvoiceIdParam) -> List[Payment]:
    db = get_db()
    rows = db.execute(
        "SELECT * FROM Payments WHERE invoice_id = ?", (params.invoice_id,)
    ).fetchall()
    db.close()
...
def pay_invoice(
    invoice_id: int, amount: float, method: str = "credit_card"
) -> Dict[str, Any]:
...
def get_customer_orders(params: CustomerIdParam) -> List[Order]:
    print("Fetching orders for customer:")
    db = get_db()
    rows = db.execute(
        """  
...
# ─── Support Tickets ────────────────────────────────────────────────────
@mcp.tool(
    description="Retrieve support tickets for a customer (optionally filter by open status)"
)
...
def create_support_ticket(
    ...
    db.commit()
    row = db.execute(
        "SELECT * FROM SupportTickets WHERE ticket_id = ?", (ticket_id,)
    ).fetchone()
    db.close()
...
def update_subscription(
    subscription_id: int, update: SubscriptionUpdateRequest
) -> dict:
    ...
    db = get_db()
    cur = db.execute(
        f"UPDATE Subscriptions SET {sets} WHERE subscription_id = ?", params
    )
    db.commit()
    db.close()
    if cur.rowcount == 0:
